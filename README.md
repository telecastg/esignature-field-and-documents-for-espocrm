...
NOTE: If you are trying to add an eSignature field to a core EspoCRM entity (like 'account' or 'contact', etc) do not modify those  entities, instead create a custom entity that clones the core entity and use that custom entity in your application in order to make the changes "upgrade safe" .
